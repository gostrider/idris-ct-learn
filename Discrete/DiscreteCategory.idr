...
-- Category "a" only has 1 morphism, which is identity
-- For this category, any object x & object y are the same category.
public export
...
discreteAssociativity : (w, x, y, z : a)
  -> (f : DiscreteMorphism w x) -> (g : DiscreteMorphism x y) -> (h : DiscreteMorphism y z)
  -> discreteCompose w x z f (discreteCompose x y z g h) = discreteCompose w y z (discreteCompose w x y f g) h
